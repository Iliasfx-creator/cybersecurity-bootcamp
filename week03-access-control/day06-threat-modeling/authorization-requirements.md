...
**When** the client adds `tenant_id`, `owner_id`, `role`, or support-only
properties

**Then** the server must reject or ignore those properties and must derive
security attributes from trusted server-side state.
...
**When** any bound value is changed, the token expires, is revoked, or has
...
**Then** the supplied value must not change the actor's authorized tenant
context.
...
**Then** the authorization must be limited to the approved object, operation
and short validity period.
